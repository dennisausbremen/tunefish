var tunefish = angular.module('tunefishApp', ['ngRoute', 'angularSoundManager', 'cfp.hotkeys']);
...
tunefish.controller('MainCtrl', function ($scope, $location, $filter, $http, $rootScope, BandFactory, BandsFactory, angularPlayer, hotkeys) {
    ...
    $scope.showRandomUnvoted = function () {
        BandsFactory.getBands().then(function (bands) {
            var selectedBands = $filter('filter')(bands, function (band) {
                return band.rating == 0
            });
    ...
    hotkeys.add({
        combo: 'r',
        description: 'Zufällige, neue Band',
        callback: function () {
            $scope.showRandomUnvoted();
        }
    });
